// ─────────────────────────────────────────────────────────────
// Wasfaty's own bootstrap — Flutter uses this file instead of the
// generated default whenever web/flutter_bootstrap.js exists.
//
// TWO REASONS IT EXISTS, both from the owner's load-time report:
//
// 1. CANVASKIT COMES FROM THIS ORIGIN, NOT FROM gstatic.com.
//    The default build fetches ~1.5–2 MB of CanvasKit wasm from
//    www.gstatic.com. That is a THIRD ORIGIN to resolve, handshake and
//    trust before a clinic in Iraq can draw its first frame — and if
//    that host is slow, filtered or blocked, the app does not start at
//    all. The engine's own copy is already inside the build output, so
//    pointing at it costs nothing and removes an external dependency
//    from the startup path of a health application.
//
// 2. THE PAGE SAYS SOMETHING WHILE IT LOADS. The report measured
//    4–5 MB before interactive and 20–60 s of blank screen on
//    clinic-grade 3G. A white page for a minute reads as broken; the
//    splash in index.html is removed by the callback below, so it is
//    tied to the app actually starting rather than to a guessed delay.
// ─────────────────────────────────────────────────────────────
...
_flutter.loader.load({
  config: {
    // Relative, so it resolves under the deploy's base href — the
    // preview is served from /wasfaty-preview/, not from the root.
    canvasKitBaseUrl: "canvaskit/",
  },
  onEntrypointLoaded: async function (engineInitializer) {
    const appRunner = await engineInitializer.initializeEngine();
    await appRunner.runApp();
    // The first frame is up: take the splash down. Tied to the app, not
    // to a timer that would either flash or linger.
    const splash = document.querySelector('#wasfaty-splash');
    if (splash) {
      splash.classList.add('wasfaty-splash--done');
